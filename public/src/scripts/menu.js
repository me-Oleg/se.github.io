document.addEventListener("DOMContentLoaded", function () {
  ...
  const menuItems = {
    admin: [
      ...
      },
      {
        ...
        submenu: [
          { name: "Страницы", link: "/pl/user/user/index" },
      ...
      },
      {
        ...
        submenu: [
          ...
          { name: "Процессы", link: "menu-item-processes" },
    ...
    student: [
      ...
      {
        name: "Сообщения",
        link: "/pl/talks/conversation",
        icon: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="17" height="17"><path d="M23.954,5.542,15.536,13.96a5.007,5.007,0,0,1-7.072,0L.046,5.542C.032,5.7,0,5.843,0,6V18a5.006,5.006,0,0,0,5,5H19a5.006,5.006,0,0,0,5-5V6C24,5.843,23.968,5.7,23.954,5.542Z"/><path d="M14.122,12.546l9.134-9.135A4.986,4.986,0,0,0,19,1H5A4.986,4.986,0,0,0,.744,3.411l9.134,9.135A3.007,3.007,0,0,0,14.122,12.546Z"/></svg>`
      },
      {
        name: "Покупки",
        link: "/sales/control/userProduct/my",
        icon: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="17" height="17"><path d="M24,3H4.242L4.2,2.649A3,3,0,0,0,1.222,0H0V2H1.222a1,1,0,0,1,.993.883L3.8,16.351A3,3,0,0,0,6.778,19H20V17H6.778a1,1,0,0,1-.993-.884L5.654,15H21.836Z"/><circle cx="7" cy="22" r="2"/><circle cx="17" cy="22" r="2"/></svg>`
      }
    ],
  };
  ...
  const controls = [
    {
      action: "toggleControlPannel",
      icon: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="17" height="17"><path d="M17.921,1.505a1.5,1.5,0,0,1-.44,1.06L9.809,10.237a2.5,2.5,0,0,0,0,3.536l7.662,7.662a1.5,1.5,0,0,1-2.121,2.121L7.688,15.9a5.506,5.506,0,0,1,0-7.779L15.36.444a1.5,1.5,0,0,1,2.561,1.061Z"/></svg>`
    },
    {
      name: "Настройки",
      icon: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="17" height="17"><path d="M17.921,1.505a1.5,1.5,0,0,1-.44,1.06L9.809,10.237a2.5,2.5,0,0,0,0,3.536l7.662,7.662a1.5,1.5,0,0,1-2.121,2.121L7.688,15.9a5.506,5.506,0,0,1,0-7.779L15.36.444a1.5,1.5,0,0,1,2.561,1.061Z"/></svg>`,
      submenu: [
        {
          action: "A+",
          link: "/teach/control/stream"
        },
        {
          action: "A-",
          link: "/pl/teach/control/schedule"
        },
        {
          action: "setTheme",
          link: "/teach/control/answers/unanswered"
        }
      ]
    }
  ];
  ...
  let currentMenu = [];
  // Проверяем наличие классов для определения статуса пользователя
  if (mainContent.classList.contains("gc-user-admin")) {
    currentMenu = menuItems.admin;
  } else if (mainContent.classList.contains("gc-user-logined")) {
    currentMenu = menuItems.user;
  } else {
    currentMenu = menuItems.guest;
  }


// Генерируем HTML для управления
const generateControlsHTML = () => {
  return `
    <div class="controls">
      <div class="control-item" onclick="toggleControlPannel()">
        ${controls[0].icon}
      </div>
...
const generateMenuHTML = (items) => {
  return items.map(item => {
    ...
    return `<li class="custom-menu-item"><div href="javascript:void(0)" class="menu-item-wrapper">${item.icon}${item.name}</div>${submenuHTML}</li>`;
  }).join('');
};
// Генерируем HTML для подменю
const generateSubmenuHTML = (items) => {
  return items.map(item => {
    return `<li class="custom-submenu-item"><a href="${item.link}" class="submenu-item-wrapper">${item.name}</a></li>`;
  }).join('');
};
// Применяем генерируемый HTML к меню
nativeMenu.innerHTML = `
  ${generateControlsHTML()}
  <ul class="custom-menu">${generateMenuHTML(currentMenu)}</ul>
`;
